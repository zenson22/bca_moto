...
import com.etc.bca_moto.dal.MotoDangKyDAO;
 ...
 * MotoDangKy
 */
@DAO(daoClass = MotoDangKyDAO.class)
@Caption("{%id}")
@Entity
@Table(name = "MOTO_DANG_KY", schema = "BCA_MOTO", uniqueConstraints = @UniqueConstraint(columnNames = "ID"))
public class MotoDangKy implements java.io.Serializable {

	private MotoDangKyId id;
	...
	@AttributeOverrides({
			...
			@AttributeOverride(name = "soTbDnchs", column = @Column(name = "SO_TB_DNCHS", columnDefinition = "VARCHAR2", length = 20)),
			@AttributeOverride(name = "ngayTbDnchs", column = @Column(name = "NGAY_TB_DNCHS", columnDefinition = "DATE", length = 7)),
			@AttributeOverride(name = "maNopThueTruocBa", column = @Column(name = "MA_NOP_THUE_TRUOC_BA", columnDefinition = "VARCHAR2", length = 50)),
			@AttributeOverride(name = "soLanTraBien", column = @Column(name = "SO_LAN_TRA_BIEN", columnDefinition = "NUMBER", precision = 22, scale = 0)) })
	public MotoDangKyId getId() {
		return this.id;
	}

	public void setId(MotoDangKyId id) {
		this.id = id;
	}
