...
import com.etc.bca_moto.dal.MotoDAO;
 ...
 * Moto
 */
@DAO(daoClass = MotoDAO.class)
@Caption("{%id}")
@Entity
@Table(name = "MOTO", schema = "BCA_MOTO", uniqueConstraints = @UniqueConstraint(columnNames = "ID"))
public class Moto implements java.io.Serializable {
	...
	@AttributeOverrides({
			...
			@AttributeOverride(name = "kiemtraNgayCapnhat", column = @Column(name = "KIEMTRA_NGAY_CAPNHAT", columnDefinition = "DATE", length = 7)),
			@AttributeOverride(name = "kiemtraNgayTao", column = @Column(name = "KIEMTRA_NGAY_TAO", columnDefinition = "DATE", length = 7)),
			@AttributeOverride(name = "tenChungTu", column = @Column(name = "TEN_CHUNG_TU", columnDefinition = "VARCHAR2")) })
	public MotoId getId() {
		return this.id;
	}

	public void setId(MotoId id) {
		this.id = id;
	}
