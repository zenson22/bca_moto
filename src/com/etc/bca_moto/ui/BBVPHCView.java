...
public class BBVPHCView extends XdevView {
	...
	public static Connection getConnection() {
		...
		for (final String persistenceUnit : Application.getPersistenceManager().getPersistenceUnits()) {
			Application.getSessionStrategyProvider().getRequestStartSessionStrategy(conversationables, persistenceUnit)
					.requestStart(conversationables, persistenceUnit);
			factory = Application.getPersistenceManager()
					.getEntityManagerFactory(persistenceUnit);
		}
		final EntityManager em = factory.createEntityManager();
		final Session ss = em.unwrap(Session.class);
		final SessionImpl sessionImpl= (SessionImpl) ss;
	...
	private void btnPrint_buttonClick(final Button.ClickEvent event) {
		...
		try {
			jpReport = JasperCompileManager
					.compileReport(path + "BCA_MOTO\\WebContent\\WEB-INF\\resources\\Output_Report_04.jrxml");
			// Preparing parameters
			final Map<String, Object> parameters = new HashMap<>();
			parameters.put("id",Long.parseLong(this.txtFiledNhapSoBB.getValue()));
	...
	private void btnChonTVPTGT_buttonClick(final Button.ClickEvent event) {
		...
		PopupWindow.For(tvptgtView)
				.caption("<div style='boder:0px;background-color:#0E82CF;width:100%;height:auto;"
						+ "	color:white;padding:0px 0px;'><b>Thêm nội dung<b></div>")
	...
	private void initUI() {
		...
		this.btnPrint.setPrimaryStyleName("v-btnIn");
		...
		this.btnNhapSoBB.setPrimaryStyleName("v-btnLuu");
